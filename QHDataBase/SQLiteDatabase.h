...
namespace QHDB
{
	...
	public:
		...
		@param zVfs			Name of VFS module to use\n
							NULL for default
		*/
		SQLiteDatabase(const char *filename, int flags, std::string key);
		SQLiteDatabase(const char *filename, int flags, std::string oldKey, std::string newKey, int mode);
		...
		*/
		...
		@param zVfs			Name of VFS module to use\n
							NULL for default
		*/
		void Open(const char *filename, int flags, std::string key);
		void Open(const char *filename, int flags, std::string oldKey, std::string newKey, int mode);
		...
		*/
		...
		//! Output: std::cout
		inline void ActivateTracing() const {sqlite3_trace(mDatabaseHandle, &QHDB::SQLiteDatabase::TraceOutput, 0);}
		...
		//! Note: time in ns\n
		//! Output: std::cout
		inline void ActivateProfiling() const {sqlite3_profile(mDatabaseHandle, &QHDB::SQLiteDatabase::ProfileOutput, 0);}
