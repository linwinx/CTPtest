...
void SQLiteStatement::Prepare(const char *sqlStatement)
{
	...
	if(sqlite3_prepare_v2(mDatabase->GetDatabaseHandle(), sqlStatement, -1, &mStatement, 0) != SQLITE_OK)
		QHDB_EXCEPT(sqlite3_errmsg(mDatabase->GetDatabaseHandle()), sqlite3_errcode(mDatabase->GetDatabaseHandle()));

	if(!mStatement)
		QHDB_EXCEPT("Prepare() SQL statement failed", -1);
}
...
bool SQLiteStatement::FetchRow() const
{
	...
	{
		case SQLITE_BUSY:
			QHDB_EXCEPT("FetchRow() SQLITE_BUSY", SQLITE_BUSY);
			return false;
		case SQLITE_DONE:
			return false;
		case SQLITE_ROW:
			return true;
		case SQLITE_ERROR:
			QHDB_EXCEPT(sqlite3_errmsg(mDatabase->GetDatabaseHandle()), SQLITE_ERROR);
			return false;
		case SQLITE_MISUSE:
			QHDB_EXCEPT("FetchRow() SQLITE_MISUSE", SQLITE_MISUSE);
			return false;
	}
...
{
	if(!mStatement)
		QHDB_EXCEPT("empty statement pointer", -1);
}

void SQLiteStatement::CheckDatabase() const
{
	if(!mDatabase)
		QHDB_EXCEPT("database pointer invalid", -1);
}
...
//			break;
//		case SQLITE_TOOBIG:
//			QHDB_EXCEPT("SQLITE_TOOBIG - string was larger than stated in byteLength or exceeded the default maximum length", -1);
//		default:
//			QHDB_EXCEPT(sqlite3_errmsg(mDatabase->GetDatabaseHandle()), sqlite3_errcode(mDatabase->GetDatabaseHandle()));
//	}
//}
			...
			if(!mTransactionSQL.empty() && !mTransactionSQL16.empty())
			{
				...
				while(i < transactions)
				{
					...
					else
					{
						...
						{
							QHDB_EXCEPT("CommitTransaction() transaction id not found", -1);
						}
					}
...
T SQLiteStatement::GetColumnValue(S sql, T(QHDB::SQLiteStatement::*getColumnFunc)(int columnNumber)const, T defaultReturnValue)
{
	Sql(sql);
...
{
	return GetColumnValue(sql, &QHDB::SQLiteStatement::GetColumnString, defaultReturnValue);
}
...
{
	return GetColumnValue(sql, &QHDB::SQLiteStatement::GetColumnInt, defaultReturnValue);
}
...
{
	return GetColumnValue<std::string, int64>(sql, &QHDB::SQLiteStatement::GetColumnInt64, defaultReturnValue);
}

int64 SQLiteStatement::GetSqlResultInt64(const char *sql, int64 defaultReturnValue) 
{
	return GetColumnValue<const char*, int64>(sql, &QHDB::SQLiteStatement::GetColumnInt64, defaultReturnValue);
}

int64 SQLiteStatement::GetSqlResultInt64(const wchar_t *sql, int64 defaultReturnValue) 
{
	return GetColumnValue<const wchar_t*, int64>(sql, &QHDB::SQLiteStatement::GetColumnInt64, defaultReturnValue);
}

double SQLiteStatement::GetSqlResultDouble(const std::string &sql, double defaultReturnValue) 
{
	return GetColumnValue(sql, &QHDB::SQLiteStatement::GetColumnDouble, defaultReturnValue);
}
...
void SQLiteStatement::CheckColumnNumber(int columnNumber, const std::string &functionName) const
{
    if(columnNumber < 0 || columnNumber >= sqlite3_column_count(mStatement))
        QHDB_EXCEPT(functionName + " column number does not exists", -1);
}
...
int SQLiteStatement::GetAssignedColumnNumber(const std::string &columnName) const
{
	...
	if(iter == mColumnNumberToColumnNameAssignment.end())
	{
		...
		QHDB_EXCEPT("GetAssignedColumnNumber() column name '" + columnName + "' does not exists", -1);
		return -1;
	}
...
std::string SQLiteStatement::Mprintf(const char *sql, ...)
{
	...
	if(!sqlResult)
	{
		QHDB_EXCEPT("unable to allocate enough memory to hold the resulting string", -1);
		return "";
	}
